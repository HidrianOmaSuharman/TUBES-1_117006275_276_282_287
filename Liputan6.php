...
echo '<h1>' . $entries->item(0)->nodeValue . ' LIPUTAN 6</h1>'; 
$i = 0; 
...
function getWebsiteContent($sUrl) { 
    ...
    if (! file_exists($sCacheFolder.$sFilename)) { 
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, Array('User-Agent: Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.8.1.15) Firefox/2.0.0.15')); 
        curl_exec($ch); 
        curl_close($ch); 
        fclose($fp); 
    } 
